...
if has('win32')
  " Windows用
  set guifont=VL_Gothic:h11:cSHIFTJIS
  " 行間隔の設定
  set linespace=1
...
" Firefox reload
if has('mac')
  nmap ,R :!osascript -e 'tell application "Firefox" to activate<C-v><CR>tell application "System Events" to keystroke "R" using {command down}<C-v><CR>tell application "MacVim" to activate'<CR><CR>
endif

" GL
if has('mac')
  command! -nargs=0 GlTmpBuild :call GlTmpBuild()
  function! GlTmpBuild ()
    nnoremap <F5> :GlTmpBuild<CR>
    !glcc % -o /tmp/GlTmpBuild
    !/tmp/GlTmpBuild&
  endfunction
endif
